...
uint8_t getpixel(image_t* image, unsigned int x, unsigned int y) {
...
void putpixel(image_t* image, unsigned int x, unsigned int y, uint8_t color) {
    image->pixels[(y * image->w) + x] = color;
}

void scale(image_t* src, image_t* dst, unsigned int newWidth, unsigned int newHeight) {
    int x, y;
